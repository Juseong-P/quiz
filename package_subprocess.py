...
# p3 = subprocess.run(['cat', 'test.txt'], capture_output=True, text=True, shell=True)
# print(p3.stdout)
# p4 = subprocess.run(['grep', '-n', 'test'], capture_output=True, shell=True, text=True, input=p3.stdout)
# print(p4.stdout)

# p3 = subprocess.run('cat test.txt | grep -n test', capture_output=True, text=True, shell=True)  #<- same function

# with open('out.txt', 'wb') as f:
    # out = subprocess.run(['dir'], shell=True, encoding='utf-8', capture_output=True)
    # f.write(out.stdout)
